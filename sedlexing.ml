...
(* Modified 2021 to adapt for use with channels connected to terminals *)
...
type lexbuf = {
  ...
  mutable current_chunk_size: int;
  mutable prompter: unit -> unit;
  mutable prompt_pending: bool;
}
...
let create ?(chunk_size=default_chunk_size) f = 
let lexbuf = 
    {
      empty_lexbuf with
        refill = f;
    ...
    in 
       lexbuf.current_chunk_size <- chunk_size;
       lexbuf
...
let set_chunk_size lexbuf chunk_size = 
    if chunk_size >= 1 then 
       lexbuf.current_chunk_size <- chunk_size 
    else 
       failwith "set_chunk_size: lexer chunk size must be >= 1"

let set_prompter lexbuf prompter = 
    lexbuf.prompter <- prompter; 
    lexbuf.prompt_pending <- true
...
let from_gen ?(chunk_size=default_chunk_size) s =
...
let refill lexbuf =
  if lexbuf.len + lexbuf.current_chunk_size > Array.length lexbuf.buf
  then begin
    let s = lexbuf.start_pos in
    let ls = lexbuf.len - s in
    if ls + lexbuf.current_chunk_size <= Array.length lexbuf.buf then
      Array.blit lexbuf.buf s lexbuf.buf 0 ls
    else begin
      let newlen = (Array.length lexbuf.buf + lexbuf.current_chunk_size) * 2 in
  ...
  let n = lexbuf.refill lexbuf.buf lexbuf.pos lexbuf.current_chunk_size in
  if n = 0
...
let next lexbuf =
  if (not lexbuf.finished) && (lexbuf.pos = lexbuf.len) then 
  (  if lexbuf.prompt_pending then ( lexbuf.prompter(); lexbuf.prompt_pending <- false );
     refill lexbuf
  );
...
(**************************)
  ...
  let from_channel ?(chunk_size=default_chunk_size) ic =
...
module Utf16 = struct
  ...
  let from_gen ?(chunk_size=default_chunk_size) s opt_bo =
    from_gen ~chunk_size (Helper.gen_from_char_gen opt_bo s)

  let from_stream s = from_gen @@ gen_of_stream s

  let from_channel ?(chunk_size=default_chunk_size) ic opt_bo =
