...
(** This module is roughly equivalent to the module Lexing from the
    ...
    The chunk size of a lexbuf  is the /initial/ size of the array
    used for buffering. Its default value is 512. But it is also
    the number of characters read when the buffer has to be
    replenished. (See also: set_chunk_size).
   
    For a lexer built from a channel connected to an interactive
    device it makes sense to set a chunk_size of about 1, for then
    replenishing the buffer succeeds as soon as a single character
    becomes available (on a line-buffered terminal this is when the
    newline charcter is typed). On the other hand, for a channel
    connected to a file it's more efficient to have a larger chunk
    size.
    
    Here's a useful way to set things up so that the chunk size is
    appropriate for /all/ media. It also, incidentally, causes
    prompts to be given when needed if the channel is interactive
    (on the assumption that a prompt should be given when an attempt
    is made to read from the channel just after a newline has been
    read).
    ...
    An alternative is to set the chunk_size after building the channel with
    the default size.
      ...
      if istty then
      (   Sedlexing.set_prompter lexbuf (fun () -> Format.printf "> %!");
          Sedlexing.set_chunk_size lexbuf 1;
      );        
*)
...
val set_chunk_size: lexbuf -> int -> unit
        (** Set the chunk size on a lexbuf. Intended  only for a
             hitherto-unused lexbuf. *)
        
val set_prompter: lexbuf -> (unit->unit)->unit
    (** Set the prompt function that gets invoked when input is
        required after a newline has been read (or as the first
        symbol is read). Intended for use only when the input source
...
val from_gen: ?chunk_size:int -> Uchar.t Gen.t -> lexbuf
    (** Create a lexbuf from a stream of Unicode code points. 
        (See also: {Utf8/Latin1/Utf16}.{from_channel,from_gen} ))
    *)
